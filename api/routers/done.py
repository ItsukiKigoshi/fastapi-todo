...
async def mark_task_as_done(task_id: int, db:AsyncSession = Depends(get_db)):
    ...
    if done is not None:
        raise HTTPException(status_code=400,detail="Already marked as done")
    
    return await done_crud.create_done(db,task_id)

@router.delete("/tasks/{task_id}/done", response_model=None)
async def mark_task_as_undone(task_id: int, db: AsyncSession = Depends(get_db)):
    done = await done_crud.get_done(db,task_id=task_id)
    if done is None:
        raise HTTPException(status_code=404,detail="Not yet marked as done")
